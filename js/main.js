...
let readyTimer = setTimeout(function(){},0)
let readyToStartTimer = setTimeout(function(){},0)
...
$(document).ready(function(){
  ...
  for(let i=0; i<counties.length; i++){
    ...
    $('#county').append(`<option value=${countiesCode[i]} ${selected}>${counties[i]}</option>`)
  }
  ...
  $('.stories__btn').on('click', function(e){
    ...
    $('.game__intro').addClass('game__intro--' + story)
    //$('.game').attr('class', 'section game')
    $('.game').addClass('game--' + story)
...
function changeSection(to, step){
  ...
  switch (step) {
    case 1 :
      ...
      let bgurl = `https://change.greenpeace.org.tw/2020/petition/zh-TW.2020.climate.emergency.signup.mc/images/bg-trans${bg==1 ? '' : '-'+bg}.jpg`
      $('.trans').css('background-image', `url(${bgurl})`)
